...
// import gaugauAudio from 'public/sound/gaugau.wav'
...
export default function PromoteCard({ card }) {
  const [isPlayButton, setPlayButton] = React.useState(false)
  ...
  return (
    <div className="bg-[#292734] text-white rounded-3xl pl-2">
      <div className="flex flex-row justify-between">
        <Image
          className="rounded-3xl ps-8 -translate-y-6 w- hover:-rotate-6"
          src={cardImg}
          alt="eo thich"
          width={160}
          height={160}
        />
        <button className="bg-[#7463F0] w-14 h-14 rounded-tr-3xl rounded-bl-3xl" onClick={clickHandler}>
          {isPlayButton ? (
            <div>
              <VoiceOne theme="outline" size="24" fill="#FFFFFF" className="m-auto inline-block" />
              <audio src="#"></audio>
            </div>
          ) : (
      ...
      <p className="font-sans inline-block mr-2">
        <span className="font-bold text-2xl">{coin}</span>.00 /{duration} giờ
      </p>
